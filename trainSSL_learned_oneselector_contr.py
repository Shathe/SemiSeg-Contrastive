...
def main():
    ...
    feature_memory = FeatureMemory(num_samples=labeled_samples, memory_per_class=2048, feature_size=256, n_classes=num_classes)
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        if random.random() > 0.5:
            weak_parameters["RandomScaleCrop"] = random.uniform(0.85, 1.5)
        ...
        feat_unlabeled_1, feat_unlabeled_2 = torch.split(features_joined_unlabeled, batch_size_unlabeled, dim=0)
        ...
        if contrastive_labeled_loss:
            ...
            #  consistency learning strong vs weak augmentations.
            #  For the time being with the two augmentaions

            # step one. Undo transformations to be able to compare. flip, scale, clasmixx
            pseudo_label2_contrastive = nn.functional.interpolate(pseudo_label2.unsqueeze(1).float(),
                                                                  size=(
                                                                      feat_unlabeled_1.shape[2],
                                                                      feat_unlabeled_1.shape[3]),
                                                                  mode='nearest').long().squeeze(1)

            pseudo_label1_contrastive = nn.functional.interpolate(pseudo_label1.unsqueeze(1).float(),
                                                                  size=(
                                                                      feat_unlabeled_1.shape[2],
                                                                      feat_unlabeled_1.shape[3]),
                                                                  mode='nearest').long().squeeze(1)
            ...
            # undo flips
            feat_unlabeled_2, pseudo_label2_contrastive, _ = transformsgpu.flip(
                flip=unlabeled_aug2_params["flip"], data=feat_unlabeled_2,
                target=pseudo_label2_contrastive, probs=None)

            feat_unlabeled_1, pseudo_label1_contrastive, _ = transformsgpu.flip(
                flip=unlabeled_aug1_params["flip"], data=feat_unlabeled_1,
                target=pseudo_label1_contrastive, probs=None)

            # undo scale
            if unlabeled_aug2_params["RandomScaleCrop"]:
                feat_unlabeled_2, pseudo_label2_contrastive, _ = transformsgpu.random_scale_crop(
                    scale=1. / unlabeled_aug2_params["RandomScaleCrop"],
                    data=feat_unlabeled_2, target=pseudo_label2_contrastive, probs=None,
                    ignore_label=ignore_label)
            valid_mask2 = pseudo_label2_contrastive != ignore_label

            if unlabeled_aug1_params["RandomScaleCrop"]:
                feat_unlabeled_1, pseudo_label1_contrastive, _ = transformsgpu.random_scale_crop(
                    scale=1. / unlabeled_aug1_params["RandomScaleCrop"],
                    data=feat_unlabeled_1, target=pseudo_label1_contrastive, probs=None,
                    ignore_label=ignore_label)
            valid_mask1 = pseudo_label1_contrastive != ignore_label

            # do classmix on the second augmentation  be bale to compare the two augmentations
            if "Mix" in unlabeled_aug1_params:
                MixMask = nn.functional.interpolate(unlabeled_aug1_params["Mix"].unsqueeze(1).float(),
                                            size=(feat_unlabeled_1.shape[2],feat_unlabeled_1.shape[3]),
                                             mode='bilinear', align_corners = True).squeeze(1)


                features_weak_unlabeled_1, _, _ = \
                    transformsgpu.mix(mask=MixMask, data=features_weak_unlabeled, target=None, probs=None)
            else:
                features_weak_unlabeled_1 = features_weak_unlabeled

            if "Mix" in unlabeled_aug2_params:
                MixMask = nn.functional.interpolate(unlabeled_aug2_params["Mix"].unsqueeze(1).float(),
                                            size=(feat_unlabeled_1.shape[2],feat_unlabeled_1.shape[3]),
                                             mode='bilinear', align_corners = True).squeeze(1)


                features_weak_unlabeled_2, _, _ = \
                    transformsgpu.mix(mask=MixMask, data=features_weak_unlabeled, target=None, probs=None)
            else:
                features_weak_unlabeled_2 = features_weak_unlabeled

            # Compare 1
            feat_unlabeled_1 = feat_unlabeled_1.permute(0, 2, 3, 1)
            feat_unlabeled_1 = feat_unlabeled_1[valid_mask1, ...]
            features_weak_unlabeled_1 = features_weak_unlabeled_1.permute(0, 2, 3, 1)
            features_weak_unlabeled_1 = features_weak_unlabeled_1[valid_mask1, ...]

            # compare 2
            feat_unlabeled_2 = feat_unlabeled_2.permute(0, 2, 3, 1)
            feat_unlabeled_2 = feat_unlabeled_2[valid_mask2, ...]
            features_weak_unlabeled_2 = features_weak_unlabeled_2.permute(0, 2, 3, 1)
            features_weak_unlabeled_2 = features_weak_unlabeled_2[valid_mask2, ...]

            # compute the projection and predictions
            proj2 = model.projection_head(feat_unlabeled_2)
            proj1 = model.projection_head(feat_unlabeled_1)
            pred2 = model.prediction_head(proj2)
            pred1 = model.prediction_head(proj1)

            # for aimed prototypes use EMA model
            with torch.no_grad():
                proj2_weak = ema_model.projection_head(features_weak_unlabeled_2)
                proj1_weak = ema_model.projection_head(features_weak_unlabeled_1)

            loss_pix_to_pix = pix_to_pix_prediction(pred1, proj1_weak) * 0.5 + pix_to_pix_prediction(pred2, proj2_weak) * 0.5
            loss = loss + loss_pix_to_pix
            ...
            if i_iter > 0.:  # RAMP_UP_ITERS  - 1000:
                ...
                # TODO: new learned PATCHES CONTRASTIVE
                # pasar el model o el ema model?
                feature_memory.add_features_from_sample_learned_oneselector(model, proj_labeled_features_correct, labels_down_correct, batch_size_labeled)
            ...
            if i_iter > 30:  # RAMP_UP_ITERS:
                ...
                loss_contr_labeled = contrastive_class_to_class_learned_oneselector(model, pred_labeled_features_all, labels_down_all, labeled_prediction_probs_all,
                                    batch_size_labeled, num_classes, feature_memory.memory, torch.ones_like(labeled_prediction_probs_all).cuda())
