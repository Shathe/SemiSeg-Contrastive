...
def main():
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        images, labels, _, _ = augment_samples_weak(images, labels, None, random.random() < 0.15, batch_size_labeled,
                                                    ignore_label)
        ...
        if contrastive_labeled_loss:
            ...
            if i_iter > RAMP_UP_ITERS - 1000:  # RAMP_UP_ITERS  - 1000:
                ...
                class_dist = torch.nn.functional.avg_pool2d(class_dist.float(),
                                                            kernel_size=int(round(
                                                                labeled_pred.shape[2] / labeled_features.shape[2])))
                ...
                mask_prediction_correctly = ((label_prediction_down == labels_down).float() * (
                            probability_prediction_down > 0.90).float() *
            ...
            if i_iter > RAMP_UP_ITERS:  # RAMP_UP_ITERS:
                ...
                mask_prediction_correctly = (
                            (labels_down != ignore_label).float() * (class_dist_labels_prob > 0.9).float()).bool()
                ...
                loss_contr_labeled = contrastive_class_to_class_with_negatives(proj_labeled_features_all, labels_down_all,
                                                                labeled_prediction_probs_all,
                                                                batch_size_labeled, num_classes, feature_memory.memory,
                                                                None)
                ...
                unlabeled_prediction_probs_down = nn.functional.interpolate(
                    correct_unlabeled_probs.float().unsqueeze(1),
                ...
                mask = ((joined_pseudolabels_down != ignore_label).float() * (class_dist_labels_prob > 0.9).float() * (
                            joined_maxprobs > 0.9).float()).bool()
                ...
                loss_contr_unlabeled = contrastive_class_to_class_with_negatives(proj_feat_unlabeled, joined_pseudolabels_down,
                                                                  unlabeled_prediction_probs_down,
                                                                  batch_size_unlabeled, num_classes,
                                                                  feature_memory.memory, joined_maxprobs_down)
